...
export class AutofilledProductDto {
  ...
  /**
   * `null` означает «лимита нет», и это ровно один случай — администратор.
   *
   * Держится на явном `hold.kind === 'platform'` в сервисе и обязано держаться
   * на нём дальше. До подписок признак выводился из отсутствия резерва
   * (`hold === null`), и соблазн оставить как было велик — но бесплатных веток
   * теперь три, и у двух из них резерва тоже нет, зато остаток кредитов есть и
   * форме нужен. Вернув правило «нет резерва — значит без лимита», мы отдадим
   * подписчику `null` и тихо погасим счётчик кредитов в форме товара: ничего
   * не упадёт, в логах не появится, увидит только продавец.
   */
  @ApiProperty({
    type: Number,
    nullable: true,
 ...
 *
 * Старые поля `price`, `allowed` и `balance` остались на своих местах, но
 * `allowed` сменил смысл целиком: раньше это было «хватает кредитов», теперь —
 * «нажатие пройдёт». При живой подписке START с непустой нормой оно `true` и
 * при нулевом балансе. Тип не изменился, компилятор про подмену смысла
 * промолчит — поэтому чтение `allowed` на клиенте обязано переезжать тем же
 * коммитом, что и эта спека.
 */
