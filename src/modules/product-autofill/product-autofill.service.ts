...
Узнать модель — не то же самое, что знать её характеристики. Даже если ты уверен, что перед тобой конкретная модель, не переписывай в карточку её паспортные данные по памяти: разрешение, частоту, объём, интерфейсы, вес, длину кабеля, версии стандартов пиши только тогда, когда это напечатано на коробке, корпусе или наклейке в кадре либо сказано продавцом. Продают не спецификацию с сайта производителя, а конкретную вещь с фотографии: её ревизия, комплектация и даже подлинность могут не совпадать с эталоном.

Фотография сильнее твоих знаний. Если надпись на упаковке спорит с тем, что ты помнишь о модели — «wireless» у модели, которая бывает только проводной, объём или разъём, которых у неё не бывает, — верь надписи, а не памяти. В таком случае model оставь null или напиши ровно то, что напечатано, а свойства из памяти не подставляй. Иначе выйдет описание одной вещи на фотографии другой: модерация сверяет карточку с фото и возвращает такую продавцу.

Перед ответом перечитай, что получилось: у каждой характеристики и каждого утверждения в описании должна быть опора — надпись или деталь, которую видно в кадре, либо слова продавца. Опоры нет — вычеркни.
...
- значение берётся с фотографии или из слов продавца. Значения, «типичные» для такой техники, и данные из спецификации узнанной модели не подставляй: непроверяемую характеристику лучше не писать совсем;
...
brand и model — производитель и модель, если их видно на фото или продавец назвал их сам. По форме корпуса не угадывай: нет уверенности — null. Модель, названная тобой самостоятельно, тянет за собой чужие характеристики и ломает всю карточку, поэтому null здесь — нормальный ответ, а не пробел.
  ...
  private async buildUserContent(
    ...
    return [
      ...
      ...attached.map((url) => ({
        ...
        // detail: 'high' — по превью 512×512 не прочитать ни модель на
        // корпусе, ни характеристики с коробки, а именно из них берутся
        // факты карточки. На трёх фото это несколько тысяч входных
        // токенов — дешевле, чем придуманная характеристика.
