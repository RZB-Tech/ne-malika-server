...
/**
 * Идентификатор заказа, который Click должен получить обратно в каждом ответе.
 *
 * Касса иногда сохраняет значение ссылки в `transaction_param`, но присылает
 * пустой `merchant_trans_id`. Подпись всё равно проверяется по сырому пустому
 * полю; подстановка нужна только после проверки — для поиска заказа и ответа.
 */
