...
/**
 * Первое число текущего месяца площадки, `YYYY-MM-DD`.
 *
 * Лежит здесь, а не рядом с тарифами, хотя пользуются им подписки: месяц, по
 * которому сбрасывается норма бесплатных автозаполнений
 * (`shops.autofill_period_month`), обязан совпадать с месяцем статистики день
 * в день. Две реализации разошлись бы ровно в те пять часов, когда в Ташкенте
 * уже первое число, а в UTC ещё последнее, — и продавец в этот час увидел бы
 * платную кнопку вместо положенной ему бесплатной попытки. Один календарь —
 * одна функция; `subscriptions.constants.ts` её переэкспортирует, но не
 * переписывает.
 *
 * Считается поверх `today()`, а не арифметикой над Date, по той же причине,
 * что и он: процесс живёт в UTC, и `getMonth()` первого числа в два часа ночи
 * по Ташкенту вернул бы прошлый месяц.
 */
