...
export interface CreditTxnMeta {
  ...
  estimated?: boolean;
  /**
   * Метка акции для разовых выдач — сейчас это `welcome`, приветственные
   * кредиты. По ней же проверяется, что магазин уже получал подарок: без
   * такой отметки повторный прогон миграции или пересоздание магазина
   * начислили бы его второй раз.
   */
  promo?: 'welcome';
}
