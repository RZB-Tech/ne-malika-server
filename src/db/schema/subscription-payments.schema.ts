...
export interface SubscriptionPaymentMeta {
  ...
  /**
   * Проверка кассы символической суммой: подпись, Prepare, Complete и запись
   * в журнал — всё настоящее, а выдачи нет вовсе. Ни кредитов, ни срока, ни
   * уведомления владельцу.
   *
   * Флагом, а не выводом из `plan = 'free'`: в `free` попадают ещё и строки
   * несостоявшегося Complete, у которых сумма не совпала ни с одним тарифом.
   * Различать «проверяли кассу» и «деньги ушли непонятно за что» по одному
   * лишь тарифу нельзя, а разбирать их приходится по-разному.
   */
  test?: boolean;
