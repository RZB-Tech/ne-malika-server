...
export const shops = pgTable(
  'shops',
  {
    ...
    /**
     * До какого момента этому магазину открыто окно тестовой оплаты. Пусто —
     * закрыто, и это состояние по умолчанию у всех.
     *
     * Зачем окно вообще. Тариф определяется по сумме платежа — Click не
     * сообщает, за что заплатили, — поэтому «принимать 1000 сум как тест»
     * означало бы принимать её от кого угодно и всегда. Окно превращает
     * тестовую сумму из вечно открытой двери в разовый пропуск: администратор
     * взводит его на конкретный магазин, оно живёт полчаса и закрывается на
     * первом же успешном Complete.
     *
     * Момент, а не флаг: флаг пришлось бы снимать заданием по расписанию, а
     * забытый взведённым флаг — это и есть та самая дверь. Просроченное окно
     * не требует ничего: сравнение с `now()` само отвечает «закрыто».
     */
